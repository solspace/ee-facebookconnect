...
{
	const FBC_PREFERENCES     = 'fbc_app_id|fbc_secret|fbc_eligible_member_groups|fbc_member_group|fbc_account_activation|fbc_passive_registration|fbc_confirm_account_sync';
	const FBC_PARAMS_LOCATION = 'db';
...
<div id="fb-root"></div>' . NL .
	...
	FB.init({' .
			'appId:"' . $this->getFacebookAppId() . '", version:"v2.5", cookie:true, status:true, xfbml:true, oauth:true' .
			'});' . NL .
		'};' . NL .
			"(function(d, s, id){
	...
	public function get_param( $tag_hash = '', $param = '', $delete = 'nodelete' )
	{
		...
		if ( self::FBC_PARAMS_LOCATION == 'cookie' )
		{
			// --------------------------------------------
		...
		if ( $this->cached[$cache_name][$param] !== FALSE )
		{
			...
			if ( $delete != 'nodelete' )
			{
				...
				if ( self::FBC_PARAMS_LOCATION != 'cookie' AND empty( $this->cached['param_deleted'][$tag_hash] ) )
				{
	...
	public function set_facebook_user_id_for_member_id( $uid = '', $member_id = '', $unsync = '' )
	{
		...
		// Check if a member isn' a super-admin, we prevent those from syncing with facebook
		// For security reasons

		$sql   = 'SELECT group_id FROM exp_members WHERE member_id = ' . ee()->db->escape_str($member_id);
		$query = ee()->db->query($sql);

		$memberGroupId = (int)$query->row('group_id');
		if ($memberGroupId === 1) {
			return false;
		}
	...
	public function set_preference( $preferences = array(), $site_id = '' )
	{
		...
		foreach ( explode( "|", self::FBC_PREFERENCES ) as $val )
		{
    ...
    /**
     * Returns legacy preferences from system preferences table
     *
     * @return array
     */
    public function getLegacyPreferences()
    {
        $sql   = "SELECT site_id, site_system_preferences FROM exp_sites";
        $query = ee()->db->query($sql);
        ...
        $preferenceKeys = explode('|', self::FBC_PREFERENCES);
        $fbPreferences = array();
        foreach ($results as $row) {
            $data = unserialize(base64_decode($row['site_system_preferences']));

            $viableData = array_intersect_key($data, array_flip($preferenceKeys));

            $fbPreferences[$row['site_id']] = $viableData;
        }
