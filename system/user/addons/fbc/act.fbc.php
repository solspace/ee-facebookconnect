...
class Fbc_actions extends AddonBuilder
{
	public $api;
	...
	public function api()
	{
		...
		$this->api = new \Solspace\Addons\Fbc\Library\Fbc_api();
	}
	...
	public function ee_login( $member_id = '' )
	{
		...
		$expire = ( isset( $this->api->user['expires'] ) === TRUE AND is_numeric( $this->api->user['expires'] ) === TRUE ) ? $this->api->user['expires'] - time() : 86400;
		...
		if ( ( $member_data = $this->model('Data')->get_member_data_from_member_id( $member_id ) ) === FALSE )
		{
			return FALSE;
		}
	...
	public function passive_registration( $uid = '' )
	{
		...
		if ( $this->model('Data')->get_member_id_from_facebook_user_id( $uid ) )
		{
